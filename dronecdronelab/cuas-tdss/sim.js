...
(function (root, factory) {
  ...
  const ZONES = {
    ...
    jammer: { x: -250, y: 180, range_m: 1500 },
    team: { x: 420, y: -300 },
  ...
  function makeDrone(opt) {
    return Object.assign({
      ...
      est_type: "quad_small", payload: "unknown", payloadConf: 0, massEst: null, swarm: null, whitelisted: false,
  ...
  const SCENARIOS = {
    approach_dive: {
      name: "직진 접근 → 급강하 (D-07)",
      desc: "남서 2.5km에서 보호대상 직진, 600m부터 급강하. RID·승인 없음, 1.5km 이내 EO/IR 추가 포착.",
      build: () => [makeDrone({ id: "D-07", x: -1500, y: -2000, alt: 120, speed: 18.4, heading: 37, script: "dive" })],
    },
    loiter_camera: {
      name: "촬영 드론 배회",
      desc: "북동 900m 상공 선회. RID 일치·승인 有, 카메라 페이로드. 등급이 과도하게 오르지 않아야 정상.",
      build: () => [makeDrone({ id: "D-12", x: 640, y: 640, alt: 80, speed: 8, heading: 180, rid: true, ridMatched: true, approval: true, deviation: 40, payload: "camera", payloadConf: 0.8, sources: ["radar", "rf", "rid"], script: "loiter" })],
    },
    rf_silent: {
      name: "RF 침묵 자율비행 (단일 센서)",
      desc: "북측 3km에서 레이더에만 포착. 차폐 구역 통과 후 접근. 재머 무효, 점수는 감축되지 않아야 함.",
      build: () => [makeDrone({ id: "D-21", x: 300, y: 3000, alt: 60, speed: 15, heading: 186, autonomous: true, sources: ["radar"], quality: 0.62, script: "straight" })],
    },
    swarm: {
      name: "군집 3기 동측 접근",
      desc: "동측 3km에서 3기 분산 접근. 군집 엔티티로 큐에 묶여야 하며 넷건은 조기 만료.",
      build: () => [0, 1, 2].map((i) => makeDrone({ id: "S-" + (i + 1), x: 3000 + i * 120, y: -300 + i * 300, alt: 90 + i * 15, speed: 12, heading: 270 + (i - 1) * 4, swarm: "SW1", sources: ["radar", "rf"], quality: 0.75, script: "straight" })),
    },
    press_whitelist: {
      name: "언론 드론 (화이트리스트)",
      desc: "승인·RID 有, 운용관이 화이트리스트 지정. 심각 게이트 외에는 등급 상승 억제.",
      build: () => [makeDrone({ id: "P-03", x: -400, y: 500, alt: 60, speed: 3, heading: 90, rid: true, ridMatched: true, approval: true, deviation: 20, payload: "camera", payloadConf: 0.9, sources: ["radar", "rf", "rid", "eoir"], quality: 0.92, whitelisted: true, script: "hover" })],
    },
    composite: {
      ...
      build: () => [
        ...
        makeDrone({ id: "F-99", x: -2900, y: 1800, alt: 50, speed: 6, heading: 120, sources: ["rf"], quality: 0.3, script: "flicker", t0: 5 }),
      ],
    },
  };
  ...
  class Simulator {
    ...
    _advance(d, dt) {
      ...
      switch (d.script) {
        case "dive":
          ...
          if (r < 1500 && !d.sources.includes("eoir")) d.sources = ["radar", "eoir"], d.quality = 0.87;
    ...
    _message(d) {
      ...
      return {
        ...
        sources: d.sources.slice(), track_quality: d.quality, detect_count: d.detect,
